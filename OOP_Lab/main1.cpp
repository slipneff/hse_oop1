...
void printMatrix(Matrix matrix)  {
  ...
  cout<<temp<<" "<<matrix.getSize()<<endl;
  for (int i = 0; i <= matrix.getCount() / matrix.getSize(); i++) {
    for (int j = 0; j < temp && j < matrix.getSize(); j++) {
      matrix.getItem(i, j).printStation();
    }
    temp = temp - matrix.getSize();
    cout << endl;
  }
}
...
int main() {
  ...
  Station *station = &vane;
  data.setItems(*station); 
  // data.setItems(vane);
  vane.printStation();
